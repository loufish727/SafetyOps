...
|---|---|---:|
| Source/static | JavaScript parses; public seed files are empty; eighteen ordered migrations and three Edge Function sources exist. | PARTIAL: source/type/build checks pass; full hosted execution does not |
| Mocked browser | UI, responsive layouts, task-first Today/Safety Monitor behavior, role presentation, invite/recovery edge cases, hosted password-class contract, stale-session rejection, provisioning pending, Pacific/Boise location-timezone handling, inspection/form RPC contracts, committee/training/employee-tablet/PDF workflows, Drive-original access/review controls, source-collection/folder hierarchy, Oregon manufacturing-reference behavior, and selected failures behave against the fake Supabase client. | PASS: 106 passed, 8 conditional/project skips, 0 failed |
...
| Hosted versioned application/upgrade | Migration versions are recorded, reviewed migrations compile/apply, and named catalog/grant assertions match. | PASS for `016`–`018`: hosted rollback compilation, application, SHA-bound ledger entries, RLS/grant checks, candidate access/collection invariants, active hardened Drive ingestion, and active JWT-protected employee file handling. Legacy identity and hostile-role runtime tests remain open |
